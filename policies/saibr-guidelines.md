...
- *1.1* *full-file integrity*: SAIBR insertions must ensure the final file contains complete content (no placeholders like “see elsewhere” that hide necessary text).  Do not replace full file with an intended append or insert.
  - *1.1.0* Read all files before writing.  *Agent-Writable* may have been changed by user.  *Change-Request* may have been added by user.  *Version: v<R>.<T>.<E>* will always have <E> incremented by user if he/she remembers, and should always be incremented by Copilot agent, too.  
  ...
  - *1.1.4* Do not ask for an overwrite authorization on a SAIBR file.  You should be applying Search-and-Insert, not overwrites. Overwrites are only permitted with "Agent-Writable: yes".
